...
import type { ILibuilder, Chunk, CLIConfig, BuildConfig } from '@modern-js/libuild';
import { waitForEqual } from './waitForEqual';

export interface ILibuilderTest extends ILibuilder {
  getOutputByCondition(predicate: (outputFilePath: string, chunk: Chunk) => boolean): Record<string, Chunk>;
...
export const getLibuilderTest = async (params: CLIConfig, name?: string): Promise<ILibuilderTest> => {
  ...
  bundler.getOutputByCondition = function (predicate) {
    return Object.fromEntries(
      Array.from(bundler.outputChunk).filter(([outputFilePath, chunk]) => predicate(outputFilePath, chunk))
    );
  };

  bundler.getCSSOutput = function () {
    return bundler.getOutputByCondition((outputFilePath) => outputFilePath.endsWith('.css'));
  };
  ...
  bundler.getJSOutput = function () {
    return bundler.getOutputByCondition((outputFilePath) => outputFilePath.endsWith('.js'));
  };
